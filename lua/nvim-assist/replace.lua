...
local function calculate_block_similarity(
    original_lines,
    search_lines,
    start_line,
    end_line
    ...
    local lines_to_check =
        math.min(search_block_size - 2, actual_block_size - 2)
...
local function line_trimmed_replacer(content, find)
    ...
    for i = 1, #original_lines - #search_lines + 1 do
        ...
        if matches then
            ...
            table.insert(
                results,
                extract_line_range(original_lines, content, i, end_line)
            )
        end
    end
...
local function block_anchor_replacer(content, find)
    ...
    if
        max_similarity >= MULTIPLE_CANDIDATES_SIMILARITY_THRESHOLD
        and best_match
    then
        table.insert(
            results,
...
function M.replace(content, old_string, new_string, replace_all)
        ...
        for _, search in ipairs(matches) do
            ...
            if index then
                ...
                if not last_index then
                    ...
                    local result = content:sub(1, index - 1)
                        .. new_string
                        .. content:sub(index + #search)
                    return result, nil
                end
    ...
    return nil,
        "Multiple matches found. Provide more context in old_string to identify the correct match."
end
